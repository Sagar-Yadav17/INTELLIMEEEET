...
const UsersList = ({ activeChannel }) => {
  ...
  // 🔥 DELETE DM CHANNEL
  const deleteDM = async (e, targetUser) => {
    e.stopPropagation();
    e.preventDefault();
    ...
    try {
      ...
      console.log("DM deleted:", channelId);

      setSearchParams({});

    } catch (err) {
      console.error("Delete DM failed:", err);
      alert("Delete failed (maybe permission issue)");
    }
  };
        ...
        return (
          <div
            ...
            className={`flex items-center justify-between px-4 py-2 rounded-lg cursor-pointer ${
              isActive ? "bg-gray-200 border-l-4 border-purple-500" : "hover:bg-blue-50"
            }`}
          >
            {/* LEFT SIDE */}
            <div
